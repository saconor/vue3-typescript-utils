<template>
  <div
    class="position-relative datepicker"
    :class="[
      useDarkMode ? 'datepicker-dark' : 'datepicker-light',
      disabled ? 'disabled' : '',
    ]"
    :id="id"
  >
    ...
    <div
      class="dateSelector position-absolute p-2 ml-n2"
      :class="{ 'd-none': !showDateSelector }"
    >
...
export default class DatePicker extends Vue {
  @Prop({ required: false, type: Dayjs, default: dayjs() }) day: Dayjs =
    dayjs();
  @Prop({ required: false, type: Dayjs, default: dayjs() }) minDate: Dayjs =
    dayjs();
  @Prop({ required: false, type: Dayjs, default: dayjs() }) maxDate: Dayjs =
    dayjs();
  ...
  @Prop({ required: false, default: false, type: Boolean }) disableFuture =
    false;
  ...
  toggleShowDateSelector(): void {
    if (!this.disabled) {
      ...
      Object.keys(window.datepickers).forEach((datePickerKey) => {
        allDatePickers = [
          ...allDatePickers,
          ...window.datepickers[datePickerKey],
        ];
      });
  ...
  valueChange(): void {
    ...
    let buildDate =
      `${this.formatIntegerTwoDigits(
        this.daySelect
      )}.${this.formatIntegerTwoDigits(this.monthSelect)}.${this.yearSelect}` +
      ` ${this.formatIntegerTwoDigits(
        this.hourSelect
      )}:${this.formatIntegerTwoDigits(this.minuteSelect)}`;
    ...
    if (this.instance > 0) {
      const otherInstancePickerDate = window.datepickers[this.instance]
        ...
        .getValue();
      const currentStartDateGreaterThanEndDate =
        this.isStart && otherInstancePickerDate.diff(newDate, "minute") < 0;
      const currentEndDateLowerThanStartDate =
        this.isEnd && newDate.diff(otherInstancePickerDate, "minute") <= 0;

      if (
        currentStartDateGreaterThanEndDate ||
        currentEndDateLowerThanStartDate
      ) {
    ...
    this.$emit("changedate", {
      ...
      isoDate: this.selectedDate.toISOString(),
    });
  }
  ...
  mounted(): void {
    ...
    if (
      ...
      window.datepickers[this.instance].length == 2
    ) {
      throw new Error("Maximal two DatePickers can share same PickerId");
    }

    window.datepickers[this.instance] = window.datepickers[this.instance]
      ? window.datepickers[this.instance]
      : [];
