<template>
  ...
  >
    <div :id="switchId" class="d-flex position-relative">
      ...
      <div
        :id="switchId + '_check_box'"
        class="switch-box position-absolute"
        :class="{ checked: ownChecked }"
      >
...
export default class Switch extends Vue {
  ...
  @Prop({ required: false, type: String, default: "#3a9d6daa" })
  checkedColor!: string;
  @Prop({ required: false, type: String, default: "#ffffffaa" })
  ...
  initSwitch() {
    const labelTrueElement = document.getElementById(
      this.switchId + "_true_label"
    );
    const labelTrueWidth = parseFloat(
      labelTrueElement ? window.getComputedStyle(labelTrueElement).width : "0"
    );

    const labelFalseElement = document.getElementById(
      this.switchId + "_false_label"
    );
    const labelFalseWidth = parseFloat(
      labelFalseElement ? window.getComputedStyle(labelFalseElement).width : "0"
    );
